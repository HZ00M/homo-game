...
@Component
public class Table_ActivityBaseTask  {
	private final static Logger logger = LoggerFactory.getLogger(Table_ActivityBaseTask.class);
	public static String META_INFO = "__META_INFO__"; 

	public static int ROW_OFFSET = 4;

	public Table_ActivityBaseTask() {
	}

	@Data
	public static class Record_ActivityBaseTask{
	...
	final String fileName = "ActivityBaseTask";
	String finalJsonPath = "ActivityBaseTask.json";

	public List<Record_ActivityBaseTask> getConfigs() {
		return configsAndKeyToIndexPair.getKey();
	}

	Pair<List<Record_ActivityBaseTask>, Map<String, Integer>> configsAndKeyToIndexPair = new Pair<>(new ArrayList<Record_ActivityBaseTask>(), new HashMap<String, Integer>());
	...
	*/
	public void load() {
		...
		for(Map.Entry<String, Object> entry: getJsonObject().entrySet()){
            ...
            Record_ActivityBaseTask record = JSON.parseObject(value.toJSONString(), Record_ActivityBaseTask.class, disableDecimalFeature);
	 ...
	 * @return
	 */
	public Record_ActivityBaseTask get(int i){
	 ...
	 * @return
	 */
	public Record_ActivityBaseTask getRecordByKey(Integer id){
            String __key__ = id + "";
	 ...
	 * @return
	 */
	public Record_ActivityBaseTask getRecordByKey(String key){
	...
	public void ascending(){
		List<Record_ActivityBaseTask> tmpConfigs = new ArrayList<>(configsAndKeyToIndexPair.getKey());
		...
		Pair<List<Record_ActivityBaseTask>, Map<String, Integer>> tmpPair = new Pair<>(tmpConfigs, tmpKeyToIndex);
		configsAndKeyToIndexPair = tmpPair;
	}

	private Map<String, Integer> calcMapIndex(List<Record_ActivityBaseTask> tmpConfigs) {
	 ...
	 * @return
	 */
	public Record_ActivityBaseTask getMaxKey(){
        return configsAndKeyToIndexPair.getKey().stream().max(Comparator.comparingInt(Record_ActivityBaseTask::getId)).orElse(null);
	}
	 ...
	 * @return
	 */
	public Record_ActivityBaseTask getMinKey(){
        return configsAndKeyToIndexPair.getKey().stream().min(Comparator.comparingInt(Record_ActivityBaseTask::getId)).orElse(null);
	}
	/**
	 * 策略查找
	 * @param predicate
	 * @return
	 */
	public List<Record_ActivityBaseTask> findByFilter(Predicate<? super Record_ActivityBaseTask> predicate){
